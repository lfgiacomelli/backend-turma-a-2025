...
const ViagemSchema = z.object({
  ...
  fun_codigo: z.string().uuid({ message: "ID do funcionário inválido" }),
  ...
  sol_codigo: z.string().uuid({ message: "ID da solicitação inválido" }),
});

const ViagemController = {
  // Buscar todas viagens de um usuário
  async getViagensPorUsuario(req, res) {
    ...
    try {
      const result = await pool.query(
        'SELECT * FROM viagens WHERE via_usuarioId = $1 ORDER BY via_data DESC',
        [usu_codigo]
      );
  ...
  // Buscar uma viagem pelo ID da viagem
  async getViagemPorId(req, res) {
    const { via_codigo } = req.params;
  ...
  // Verificar se viagem está em andamento para um usuário
